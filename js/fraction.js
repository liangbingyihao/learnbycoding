...
function getFormulaHtml(question,sn) {
  ...
  let questionStr = question["term"].replace(/\s*'(\d+\/\d+)'/g, (match, fraction) => {
    // 将匹配到的分数字符串进行替换
    return fraction.replace(/(\d+)\/(\d+)/g, '\\frac{$1}{$2}');
  });
  let answerStr = question["resultStr"].replace(/\s*'(\d+\/\d+)'/g, (match, fraction) => {
    // 将匹配到的分数字符串进行替换
    return fraction.replace(/(\d+)\/(\d+)/g, '\\frac{$1}{$2}');
  });
  answerStr = answerStr.replace(/[\(\)]/g, '');
  return '<div class="bg-1"><span class="no-part">' + sn + '. </span><math-field readonly>'+questionStr+'</math-field></div>\
  <div style="visibility: hidden;" class="my-answer-container bg-2"><span class="no-part"> = </span><math-field readonly>' + answerStr + '</math-field></div>'
}
